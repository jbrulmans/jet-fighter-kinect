...
public class KinectControllerTwoHandsAutoPilot : MonoBehaviour, GestureListener {
	...
	public bool fullAutoPilot = true;
	...
	public void leanGesture(float aLeftRight, float aFrontBack) {
		...
		else if (angleArms < arm_mid - arm_mid_range && angleArms > arm_mid + arm_mid_range){
	...
	public void armGesture(float angleLeft, float angleRight) {
		this.angleArms = (angleLeft+angleRight)/2;
		
		autoPilot = angleArms < autoPilotThreshold;
		Debug.Log ("autopilot " + (autoPilot?"on":"off"));
		...
		} else if (angleArms > arm_mid + arm_mid_range) {
			player.autoBalancePlaneHorizontal(false);
			player.autoBalancePlaneVertical(false);
			player.moveUpOrDown (1.0f);
